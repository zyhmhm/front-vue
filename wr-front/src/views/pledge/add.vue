<template>
  <div>
      ...
      <div style="margin: 50px auto; border-bottom:1px solid #000000; border-top:1px solid #000000; text-align: center;">
        <el-form ref="dataForm" :inline="true" :rules="rules1" :model="pledge" label-position="left" label-width="100px" style="width: 70%; margin: 20px auto;">
          <el-form-item label="仓储企业" prop="company">
            <el-input v-model="pledge.company" :disabled="true"/>
          </el-form-item>
          <el-form-item label="仓库名称" prop="repoName">
            <el-input v-model="pledge.repoName"  :disabled="true"/>
          </el-form-item>
          <el-form-item label="货物名称" prop="goodsName">
            <el-input v-model="pledge.goodsName"  :disabled="true"/>
          </el-form-item>
          <el-form-item label="受让人Id" prop="toUserId">
            <el-input v-model="pledge.toUserId" />
          </el-form-item>
          <el-form-item label="质押原因" prop="reason">
            <el-input type="textarea" v-model="pledge.reason"/>
          </el-form-item>
        </el-form>
...
import { doSavePledge } from '@/api/pledge'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
  ...
  export default {
    ...
    methods: {
      //保存转让申请
      submitTransfer() {
        doSavePledge(this.$qs.stringify(this.pledge)).then(res => {
          if (res.code === 200) {
            alert('添加成功！')
            this.$router.push('/businessApplication/shipper-pledge')
          } else {
            alert(res.msg)
          }
        })
      ...
      // 选择仓单
      quiteCheck(row) {
        this.pledge.electronicWarehouseId = row.id
        this.pledge.company = row.custodian
        this.pledge.goodsName = row.goodsName
        this.pledge.repoName = row.caddr
        this.isCheck = true
      },
      // 仓单详情
